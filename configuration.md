...
DBPack 启动时，可以通过 `--config` 或者 `-c` 指定配置文件，默认为 `config.yaml`。配置内容的的代码结构为：
...
listeners:
  ...
  - protocol_type: mysql
    ...
    # 监听器的配置
    config:
      # mysql 代理的用户名和密码，map 结构，连接 DBPack 的客户端使用这里配置的用户名和密码
      users:
        dksl: "123456"
      # DBPack 代理的后端 DB 的版本
...
filters:
  ...
  - name: httpDTFilter
    ...
    conf:
      ...
      # transaction_infos 如果 http 请求的 url 和 request_path 匹配，则拦截器会执行创建全局事务的逻辑，并将 x-dbpack-xid 注入请求 header 中。
      transaction_infos:
