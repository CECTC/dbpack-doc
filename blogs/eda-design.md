...
![eda.drawio](../images/eda.drawio.png)
...
![eda-Page-2.drawio](../images/eda-Page-2.drawio.png)
...
![image-20220427100734991](../images/distributed-transaction.gif)
...
![eda-Page-3.drawio](../images/eda-Page-3.drawio.png)

事务分支提交或者回滚失败后，会重新进入到 `workqueue` 当中继续消费，直至提交、回滚成功，或回滚超时（AT 模式回滚操作涉及到全局锁的释放，需要设置超时时间，即 `retry_dead_threshold`）。

![image-20220530171829086](../images/image-20220530171829086.png)
...
hptx 是当前性能最强的云原生、无侵入分布式事务解决方案，选择其他内存型存储组件理论上可以得到更高的性能，但综合可靠性和性能，ETCD 是最好的选择。
...
欢迎感兴趣的同学加入我们的社区，在我们社区可以一起交流分布式事务问题、DBMesh 问题，进群或参与社区建设请添加微信：scottlewis。
